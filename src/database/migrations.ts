...
async function ensureContractCommissionAllocations(): Promise<void> {
  if (!(await tableExists('contracts')) || !(await tableExists('negotiations'))) {
    return;
  }

  if (!(await columnExists('contracts', 'finalized_at'))) {
    await connection.query(`
      ALTER TABLE contracts
...
async function ensureContractDocumentRejectionsTable(): Promise<void> {
  if (!(await tableExists('contracts')) || !(await tableExists('negotiations'))) {
    return;
  }
...
async function ensureAdminPanelRoles(): Promise<void> {
  if (!(await tableExists('admins'))) {
    return;
  }
