...
function isLegacySyntheticEditField(
  key: string,
  before: Record<string, unknown>,
  after: Record<string, unknown>,
): boolean {
  // Public codes are generated by the system and were never a supported
  // client-side edit. Hide stale requests created before that rule.
  if (key === 'code') return true;

  if (
    key === 'semLote' &&
    isEnabledFlag(before.semLote) &&
    isDisabledFlag(after.semLote) &&
    !String(after.lote ?? '').trim()
  ) {
    return true;
  }
  ...
  if (
    key === 'semQuadra' &&
    isEnabledFlag(before.semQuadra) &&
    isDisabledFlag(after.semQuadra) &&
    !String(after.quadra ?? '').trim()
  ) {
    return true;
  }
  ...
  return (
    key === 'valorCondominio' &&
    (before.valorCondominio == null || before.valorCondominio === '') &&
    Number(after.valorCondominio) === 0
  );
}

function sanitizeLegacySyntheticEditFields(
  before: Record<string, unknown>,
  after: Record<string, unknown>,
  diff: Record<string, unknown>,
  fieldReviews: Record<string, unknown>,
  ...
  const sanitizedFieldReviews = { ...fieldReviews };

  for (const key of Object.keys(sanitizedDiff)) {
    if (!isLegacySyntheticEditField(key, before, after)) continue;
    delete sanitizedBefore[key];
    delete sanitizedAfter[key];
    delete sanitizedDiff[key];
    delete sanitizedFieldReviews[key];
  }
  ...
  return {
    ...
    diff: sanitizedDiff,
    fieldReviews: sanitizedFieldReviews,
  };
}
...
function mapPropertyEditRequest(row: PropertyEditRequestListRow): PropertyEditRequestListItem {
  const sanitized = sanitizeLegacySyntheticEditFields(
    parseJsonObjectSafe(row.before_json),
    parseJsonObjectSafe(row.after_json),
    parseJsonObjectSafe(row.diff_json),
    parseJsonObjectSafe(row.field_reviews_json),
  );
  ...
  return {
    ...
    diff: sanitized.diff,
    fieldReviews: sanitized.fieldReviews,
